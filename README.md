# Notas de Aula de Matemática para o Mestrado e Doutorado em Economia

**EPGE/FGV · Professora Silvia Matos · 2026.1**  
**Autor:** Vitor Wilher

---

## Download

As notas são compiladas em um único PDF, atualizado ao longo do curso:

**[Ver PDF (versão mais recente)](https://github.com/vitorwilher/matematica-silvia-matos/blob/main/index.pdf)**

---

## Conteúdo

### Aulas

| # | Tópico |
|---|--------|
| 1 | Introdução |
| 2 | Nivelamento: Conjuntos e Funções |
| 3 | Limites e Continuidade |
| 4 | Derivadas |
| 5 | Antiderivadas (Integral Indefinida) |
| 6 | Integral Definida |
| 7 | Função Logaritmo Natural |
| 8 | Função Inversa |
| 9 | Funções Exponenciais |
| 10 | Integração por Partes |
| 11 | Equações Diferenciais Ordinárias |
| 12 | Funções de Várias Variáveis |
| 13 | Álgebra Linear |
| 14 | Otimização sem Restrição |
| 15 | Otimização com Restrição de Igualdade |
| 16 | Aplicações Econômicas |

### Monitorias

| # | Tópicos |
|---|---------|
| 1 | Derivadas — regras elementares e regra da cadeia |
| 2 | Derivadas — diferenciação logarítmica |
| 3 | Análise de funções — extremos e concavidade |
| 4 | Antiderivadas e mudança de variável |
| 5 | Integral definida e Teorema Fundamental do Cálculo |
| 6 | Funções logarítmica, inversa e exponencial |
| 7 | Diferenciação logarítmica e integração por partes |
| 9 | Resumo das regras de derivação e exame 2025 |
| 10 | EDO de 2ª ordem com coeficientes constantes — P1 2025 |

### Testes Anteriores

| Teste | Tópicos |
|-------|---------|
| Teste 1 | Derivadas |
| Teste 2 | Integrais |
| Teste 3 | Derivadas e integrais |

---

## Estrutura do Repositório

```
aulas/          → notas de aula por tópico (16 arquivos .qmd)
monitorias/     → exercícios resolvidos das monitorias (9 arquivos .qmd)
testes/         → provas anteriores com soluções (3 arquivos .qmd)
index.qmd       → compilador geral — gera o index.pdf
index.pdf       → PDF completo com todas as notas, monitorias e testes
referencias.bib → referências bibliográficas
```

---

## Referências Bibliográficas

- **Chiang, A. C. e Wainwright, K.** *Fundamental Methods of Mathematical Economics*, 4ª ed. McGraw-Hill, 2005.
- **Simon, C. P. e Blume, L.** *Mathematics for Economists*. W. W. Norton & Company, 1994.
- **Leithold, L.** *O Cálculo com Geometria Analítica*, Vol. 1, 3ª ed. Harbra, 1994.
- **Guidorizzi, H. L.** *Um Curso de Cálculo*, Vol. 2, 5ª ed. LTC, 2001.
- **Shilov, G. E.** *Linear Algebra*. Dover Publications, 1977.
